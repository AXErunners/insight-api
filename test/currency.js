...
describe('Currency', function() {
  ...
  var axeCentralData = {
    ...
    exchange_rates: {
      axe_usd: 9.4858840414,
      btc_usd: 682.93,
      btc_axe: 0.01388998
    }
  };

  it.skip('will make live request to axe central', function(done) {
    var currency = new CurrencyController({});
    var req = {};
    var res = {
      jsonp: function(response) {
        response.status.should.equal(200);
        should.exist(response.data.axe_usd);
        (typeof response.data.axe_usd).should.equal('number');
        done();
      }
    };
  ...
  it('will retrieve a fresh value', function(done) {
    var TestCurrencyController = proxyquire('../lib/currency', {
      request: sinon.stub().callsArgWith(1, null, {statusCode: 200}, JSON.stringify(axeCentralData))
    });
    var node = {
      log: {
        error: sinon.stub()
    ...
    var res = {
      jsonp: function(response) {
        response.status.should.equal(200);
        should.exist(response.data.axe_usd);
        response.data.axe_usd.should.equal(9.4858840414);
        done();
      }
    };
    currency.index(req, res);
